...
<body class="bg-gray-200 flex items-center justify-center min-h-screen">
    <div class="bg-white p-4 sm:p-6 md:p-8 rounded-lg shadow-lg w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl">
        <h1 class="text-lg sm:text-xl md:text-2xl font-bold mb-2 flex items-center">
            <span class="border-l-4 border-black pl-2">EDIT ABOUT</span>
        </h1>
        <p class="text-gray-600 mb-6">Update the details to edit the about section</p>
        <form action="{{ route('about.update', $about->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label class="block text-gray-700 mb-2" for="title">Title</label>
                <input class="w-full px-3 py-2 border border-gray-300 rounded" type="text" id="title" name="title" value="{{ $about->title }}" placeholder="Enter the title">
            </div>
            <div class="mb-6">
                <label class="block text-gray-700 mb-2" for="description">Description</label>
                <textarea class="w-full px-3 py-2 border border-gray-300 rounded" id="description" name="description" rows="4" placeholder="Enter the description">{{ $about->description }}</textarea>
            </div>
